...
//
// Force / soft update gate for the DRIVER app. On launch (and on resume) it asks
// the backend for the minimum required version for this platform + app, compares
// it against its own installed versionName, and decides whether to show a blocking
// ("required") or dismissible ("available") update modal.
//
// FAIL-OPEN: any network/parse error resolves to { status: 'none' } so a backend
// outage can never lock drivers out of the app.
...
/** Compare dotted numeric version strings ("1.6" vs "1.7"). -1 / 0 / 1. */
...
/**
 * Ask the backend whether this driver install must / should update.
...
export async function checkForUpdate(): Promise<UpdateCheckResult> {
  try {
    ...
    const res = await fetch(
      `${API_CONFIG.BASE_URL}/app/config?platform=${platform}&app=driver`,
      { method: 'GET' },
    );
